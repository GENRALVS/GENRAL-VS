...
local function check_member_super(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = 'this bot has been added✅ in Group🔸'..msg.to.title
...
return 'Links is already locked 🔐\nIn with Mister🔸 :️ @'..msg.from.username  else
    data[tostring(target)]['settings']['lock_link'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'Links has been locked 🔐\nIn with Mister🔸 :️ @'..msg.from.username
  end
end
...
local function unlock_group_leave(msg, data, target)
  ...
  if group_leave_lock == 'no' then
    ...
    save_data(_config.moderation.data, data)
    return 'leave has been unlocked 🔓\nn with Mister🔸 :️ @'..msg.from.username
  end
end
...
local function unlock_group_operator(msg, data, target)
  ...
  if group_operator_lock == 'no' then
    ...
    save_data(_config.moderation.data, data)
return 'operator has been unlocked 🔓\nIn with Mister🔸 : @'..msg.from.username  end
end
...
local function unlock_group_reply(msg, data, target)
  ...
  if group_reply_lock == 'no' then
    ...
    save_data(_config.moderation.data, data)
    return 'Reply is not locked 🔓\nIn with Mister🔸 :️ @'..msg.from.username  end
end
...
local function promote2(receiver, member_username, user_id)
  ...
  local member_tag_username = string.gsub(member_username, '@', '(at)')
  if not data[group] then
    return send_large_msg(receiver, 'SuperGroup is not added in ❌ Group🔸')
  end
  if data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_username..' is already a moderator ✅💭')
  end
...
local function modlist(msg)
  ...
  if not data[tostring(groups)][tostring(msg.to.id)] then
    return 'SuperGroup is not added in ❌ Group🔸'
  end
...
local function DevPointTeam(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		  ...
		  local iDev1 = "is already added ✅ in Group🔸"
		...
		if matches[1] == 'rem' and is_admin1(msg) and not matches[2] then
			if not is_super_group(msg) then
				return reply_msg(msg.id, 'SuperGroup is not added in ❌ Group🔸', ok_cb, false)
			end
			print("SuperGroup "..msg.to.print_name.."("..msg.to.id..") removed")
			superrem(msg)
			rem_mutes(msg.to.id)
		...
		if matches[1] == "owner" then
			...
			if not group_owner then
								return "no owner ⚠️ ,ask admins in support groups to set owner for your SuperGroup ❎⚠️"
			end
...
return "SuperGroup owner is 🔶 ["..group_owner..']'
		...
		if matches[1] == "wh" and not matches[2] and is_momod(msg) then
		...
		if matches[1] == 'id' then
			...
			elseif msg.text:match("@[%a%d]") then
				...
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] requested SuperGroup ID")
 				local text = "♦️ Name: " ..string.gsub(msg.from.print_name, "_", " ").. "\n♦️USERNAME: @"..(msg.from.username or '----').."\n♦️ID : "..msg.from.id.."\n♦️Group Name : " ..string.gsub(msg.to.print_name, "_", " ").. "\n♦️CHAT ID : "..msg.to.id
        return reply_msg(msg.id, text, ok_cb, false)
			end
		end
		...
		if matches[1] == 'nwlnk' and is_momod(msg)then
			local function callback_link (extra , success, result)
			local receiver = get_receiver(msg)
				if success == 0 then
					send_large_msg(receiver, 'Error : Failed to retrieve link 💭⁉️  \nReason: Not creator 🔷💭 \n\nIf you have the link, please use /slnk to set it ✅💭')
       ...
       return 'Please send the new group link now 🔸💭'
		...
		if matches[1] == 'lnk' then
			...
			if not group_link then
				return "reate a link using/nwlnk first ✅\nOr if I am not creator use /slnk to set your link 🔸💭"
			end
		...
		if matches[1] == 'upro' then
			...
			elseif matches[1] == 'upro' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'demoteadmin'
				user_info(user_id, cb_user_info, {receiver = receiver, get_cmd = get_cmd})
			elseif matches[1] == 'upro' and not string.match(matches[2], '^%d+$') then
				local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'demoteadmin'
				}
		...
		if matches[1] == 'sown' and is_owner(msg) then
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'setowner',
					msg = msg
		...
		if matches[1] == 'sdm' then
		  if not is_momod(msg) then
			...
			elseif matches[1] == 'sdm' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'promote'
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] promoted user#id"..matches[2])
				user_info(user_id, cb_user_info, {receiver = receiver, get_cmd = get_cmd})
			elseif matches[1] == 'sdm' and not string.match(matches[2], '^%d+$') then
				local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'promote',
				}
		...
		if matches[1] == 'udm' then
			...
			elseif matches[1] == 'udm' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
		...
		if matches[1] == "stnm" and is_momod(msg) then
			local receiver = get_receiver(msg)
		...
		if matches[1] == "stat" and is_momod(msg) then
			local receiver = get_receiver(msg)
		...
		if matches[1] == 'strs' and is_momod(msg) then
			rules = matches[2]
			local target = msg.to.id
		...
		if matches[1] == 'stpho' and is_momod(msg) then
		...
		if matches[1] == 'rm' then
			...
			if matches[2] == 'usnm' and is_admin1(msg) then
				local function ok_username_cb (extra, success, result)
					local receiver = extra.receiver
					if success == 1 then
		...
		if matches[1] == 'k' and is_momod(msg) then
			local target = msg.to.id
		...
		unlock_group_operator(msg, data, target),
      	...
      	return lock_group_etehad(msg, data, target), etehad
			...
			if matches[2] == 'ara' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked arabic ")
				return lock_group_arabic(msg, data, target)
			end
			if matches[2] == 'mbm' then
			...
			if matches[2] == 'tgse' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked Tgservice Actions")
				return lock_group_tgservice(msg, data, target)
			end
			if matches[2] == 'stk' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked sticker posting")
				return lock_group_sticker(msg, data, target)
			end
			if matches[2] == 'cots' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked contact posting")
				return lock_group_contacts(msg, data, target)
			end
			if matches[2] == 'str' then
		...
		if matches[1] == 'm' and is_owner(msg) then
			local chat_id = msg.to.id
			...
			local msg_type = 'Audio'
				if not is_muted(chat_id, msg_type..': yes') then
					...
					mute(chat_id, msg_type)
					    return 'Audio has been muted 🎵🔐\nIn with Mistery🔸 :️ @'..msg.from.username
				else
					    return 'Audio is already muted 🎵🔐\nIn with Mister🔸 :️ @'..msg.from.username
				end
			end
			if matches[2] == 'pho' then
		...
		if matches[1] == "sil" or matches[1] == "usil" and is_momod(msg) then
			local chat_id = msg.to.id
			local hash = "mute_user"..chat_id
			local user_id = ""
			if type(msg.reply_id) ~= "nil" then
			...
			elseif matches[1] == "sil" or matches[1] == "usil" and string.match(matches[2], '^%d+$') then
				local user_id = matches[2]
				if is_muted_user(chat_id, user_id) then
					unmute_user(chat_id, user_id)
			...
			elseif matches[1] == "sil" or matches[1] == "usil" and not string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local get_cmd = "mute_user"
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
		...
		if matches[1] == "media" and is_momod(msg) then
			local chat_id = msg.to.id
		...
		if matches[1] == 'ses' and is_momod(msg) then
			local target = msg.to.id
