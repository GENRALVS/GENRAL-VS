...
local function action_by_reply(extra, success, result)-- (reply) /info  function
  if result.from.username then
    ...
    local text = '💭- First name : '..(result.from.first_name or '')..' '..(result.from.last_name or '')..'\n'
               ..'💭- Username : '..Username..'\n'
  ...
  local value = redis:hget(hash, result.from.id)
   if not value then
      if result.from.id == tonumber(Arian) then
         text = text..'▫️- whois: (Sudo) \n\n'
       elseif is_admin2(result.from.id) then
         text = text..'▪️- whois: (Admin) \n\n'
       elseif is_owner2(result.from.id, result.to.id) then
         text = text..'▫️- whois: (Owner) \n\n'
       elseif is_momod2(result.from.id, result.to.id) then
         text = text..'▪️- whois: (Moderator) \n\n'
   else
         text = text..'▫️- whois: (Member) \n\n'
   end
    else
     text = text..'› whois : '..value..'\n\n'
   end
...
local function run(msg, matches)
 ...
 if matches[1]:lower() == 'info' and not matches[2] then
  ...
  if msg.from.username then
   ...
   local text = '💭- First Name : '..(msg.from.first_name or 'لا يوجد ')..'\n'
   local text = text..'💭- Last Name : '..(msg.from.last_name or 'لا يوجد ')..'\n' 
 ...
 if hash then
  ...
  elseif is_owner(msg) then
   text = text..'💭- Your : --owner-- \n'
  elseif is_momod(msg) then
   text = text..'💭- Your : --admin-- \n'
  else
   text = text..'💭- Your : --members--\n'
  end
   else
    text = text..'💭 whois : '..value..'\n'
   end
